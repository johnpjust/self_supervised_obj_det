...
def default_detection_configs():
  ...
  in BiFPN, the input resolution must be dividable by 2^7 =
  128, so we linearly increase resolutions using equation:
  Rinput = 512 + φ · 128 (3)
  Following Equations 1,2,3 with different φ, we have developed EfficientDet-D0 (φ = 0) to D7 (φ = 7) as shown
  in Table 1, where D7 and D7x have the same BiFPN and
  head, but D7 uses higher resolution and D7x uses larger
  backbone network and one more feature level (from P3 to P8).
  '''
  ...
  h.anchor_scale = 4.0 # question:  what is this?
  # is batchnorm training mode
  h.is_training_bn = True
  # optimization
  h.momentum = 0.9 ## this is for SGD optimizer
  ...
  # localization loss
  h.delta = 0.1  # regularization parameter of huber loss.  Point where square changes to abs?
  # total loss = box_loss * box_loss_weight + iou_loss * iou_loss_weight
  h.box_loss_weight = 50.0
  h.iou_loss_type = None    # ('iou', 'ciou', 'diou', 'giou')
  ...
  h.nms_configs = {
      'method': 'gaussian', # options {'gaussian', 'linear'}???  Soft NMS.
